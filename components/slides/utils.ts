...
// Resolve theme colors to actual values
export const resolveColor = (
  ...
  if (color.startsWith("theme.")) {
    // Map theme tokens to CSS variables or default colors
    ...
    return themeMap[color] || fallback;
  }
  return color;
};

// Convert fill to CSS background
export const fillToCSS = (fill: Fill | undefined): React.CSSProperties => {
  if (!fill) return {};
...
// Convert effects to CSS
export const effectsToCSS = (
...
// Get framer-motion animation config
export const getAnimationConfig = (animation: Animation | undefined) => {
  ...
  const ease = Array.isArray(animation.easing)
    ? animation.easing
    : easingMap[animation.easing || "ease-out"] || "easeOut";

  const animationConfigs: Record<string, { initial: object; animate: object }> =
    {
  ...
  const config =
    animationConfigs[animation.type] || animationConfigs["fade-in"];
...
// Get base element styles
export const getBaseElementStyles = (
